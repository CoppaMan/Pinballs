...
class CollisionSim : public Simulation {
   public:
    ...
    const int NUM_CUBES = 4;  // complexity of scene
    ...
    virtual void resetMembers() override {
        ...
        if (NUM_CUBES > 1) {
            m_objects[1].setPosition(Eigen::Vector3d(10, 8, 0));
            color << 204.0 / 255.0, 0.0, 0.0;
            m_objects[1].setColors(color);
        }
        if (NUM_CUBES > 2) {
            m_objects[2].setPosition(Eigen::Vector3d(10, 11, 0));
            color << 204.0 / 255.0, 102.0 / 255.0, 0.0;
            m_objects[2].setColors(color);
        }
        if (NUM_CUBES > 3) {
            m_objects[3].setPosition(Eigen::Vector3d(10, 14, 0));
            color << 204.0 / 255.0, 204.0 / 255.0, 0.0;
            m_objects[3].setColors(color);
        }
        if (NUM_CUBES > 4) {
            m_objects[4].setPosition(Eigen::Vector3d(13, 8, 0));
            color << 102.0 / 255.0, 204.0 / 255.0, 0.0;
            m_objects[4].setColors(color);
        }
        if (NUM_CUBES > 5) {
            m_objects[5].setPosition(Eigen::Vector3d(13, 11, 0));
            color << 0.0, 204.0 / 255.0, 0.0;
            m_objects[5].setColors(color);
        }
        if (NUM_CUBES > 6) {
            m_objects[6].setPosition(Eigen::Vector3d(13, 14, 0));
            color << 0.0, 204.0 / 255.0, 102.0 / 255.0;
            m_objects[6].setColors(color);
        }
        if (NUM_CUBES > 7) {
            m_objects[7].setPosition(Eigen::Vector3d(16, 8, 0));
            color << 0.0, 204.0 / 255.0, 204.0 / 255.0;
            m_objects[7].setColors(color);
        }
        if (NUM_CUBES > 8) {
            m_objects[8].setPosition(Eigen::Vector3d(16, 11, 0));
            color << 0.0, 102.0 / 255.0, 204.0 / 255.0;
            m_objects[8].setColors(color);
        }
        if (NUM_CUBES > 9) {
            m_objects[9].setPosition(Eigen::Vector3d(16, 14, 0));
            color << 0.0, 0.0, 204.0 / 255.0;
            m_objects[9].setColors(color);
        }
        m_objects.back().setScale(100);
        m_objects.back().setType(ObjType::STATIC);
        m_objects.back().setPosition(Eigen::Vector3d(0, -105, 0));
        color << 0.5, 0.5, 0.5;
        m_objects.back().setColors(color);

        for (size_t i = 0; i < m_objects.size(); i++) {
            m_objects[i].setMass(m_mass);
        }
        m_objects.back().setMass(std::numeric_limits<double>::max());
        updateVars();
    }
